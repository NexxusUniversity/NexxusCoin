...
bool static PoWMiner(CWallet *pwallet, CBlock *pblock){
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    while (true)
    {
        ...
        {
            fTryToSync = false;
            if (vNodes.empty() || pindexBest->GetBlockTime() < GetTime() - 10 * 60)
            {
                MilliSleep(60000);
                continue;
            }
        ...
        //
        int64_t nFees;
        bool proofOfStake = pindexBest->nHeight >= Params().LastPOWBlock();
        auto_ptr<CBlock> pblock(CreateNewBlock(reservekey, proofOfStake, &nFees));
        if (!pblock.get())
            return;
        ...
        if (proofOfStake)
        {
            ...
            if (PoWMiner(pwallet, pblock.get()))
            {
                SetThreadPriority(THREAD_PRIORITY_NORMAL);
