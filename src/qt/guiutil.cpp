...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Nexxuscoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::NXX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert nexxuscoin:// nexxuscoinuscoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("nexxuscoin://"))
    {
        uri.replace(0, 12, "nexxuscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NexxusCoin.lnk";
}
...
{
    return GetAutostartDir() / "nexxuscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NexxusCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  nexxuscoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("NexxusCoin-Qt"));
