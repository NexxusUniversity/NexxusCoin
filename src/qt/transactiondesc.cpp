...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    else if (GetTransaction(prevOut.hash, prevTx, prevBlockHash))
    {
        CScript prevScript = prevTx.vout[prevOut.n].scriptPubKey;
        CTxDestination txDestination;
        if (ExtractDestination(prevScript, txDestination))
        {
            CBitcoinAddress addrFrom;
            addrFrom.Set(txDestination);
            strHTML += "<b>" + tr("From") + ":</b> " + GUIUtil::HtmlEscape(addrFrom.ToString()) + "<br>";
        }
        // Online transaction
    }
    else
